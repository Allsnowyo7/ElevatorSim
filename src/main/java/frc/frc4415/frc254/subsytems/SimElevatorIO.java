...
public class SimElevatorIO extends TalonFXIO {
    ...
    protected void updateSimState() {
        ...
        DogLog.log(config.name + "/Sim/SimulatorVoltage", simVoltage);
        ...
        DogLog.log(config.name + "/Sim/SimulatorPositionMeters", simPositionM);
        ...
        DogLog.log(config.name + "/Sim/setRawRotorPosition", rotorPosition);

        // Mutate rotor vel
        double rotorVel = Units.metersToInches(sim.getVelocityMetersPerSecond()) / config.unitToRotorRatio;
        simState.setRotorVelocity(rotorVel);
        DogLog.log(config.name + "/Sim/SimulatorVelocityMS", sim.getVelocityMetersPerSecond());
    }
}